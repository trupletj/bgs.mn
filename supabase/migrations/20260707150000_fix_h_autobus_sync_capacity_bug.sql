-- Fix two bugs found in handle_h_autobus_sync():
-- 1. Capacity was hardcoded to 45 instead of using the real seat count
--    from target.h_autobus.number_person (falls back to 45 when null).
-- 2. capacity was in the INSERT column list but missing from the
--    ON CONFLICT ... DO UPDATE SET list, so it silently never updated
--    after the first insert for a given h_autobus_id.
