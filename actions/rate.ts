...
export async function submitReview(params: SubmitReviewParams) {
  ...
  try {
    ...
    if (params.status === "changes_requested" && params.newQuantities) {
      const affectedIds = Object.keys(params.newQuantities).map(Number);

      if (affectedIds.length > 0) {
        const subItems = affectedIds.map((id) => ({
          order_item_id: id,
          quantity: params.newQuantities![id],
          status: "changes_requested",
          description:
            params.itemComments?.[id]?.trim() ||
            params.comments.trim() ||
            "Өөрчлөлт шаардлагатай",
    ...
    if (params.status === "rejected") {
      ...
      await supabase
        .from("orders")
        .update({ status: "rejected", rejected_at: now, completed_at: now })
        .eq("id", order_id);
    }
    ...
    return { success: false, error: "Үйлдэл амжилтгүй боллоо" };
  }
}
...
async function performChecks(params: SubmitReviewParams) {
  ...
  if (order_steps.step_order !== order_instances.current_step_order) {
    return { success: false, error: "Буруу алхам дээр байна." };
  }
  ...
  if (params.status === "changes_requested") {
    ...
    const hasItemComments =
      params.itemComments &&
      Object.values(params.itemComments).some((c) => c.trim().length > 0);
    const hasGeneralComment = params.comments.trim().length > 0;

    if (!hasQuantityChange && !hasItemComments && !hasGeneralComment) {
      return {
        success: false,
        error:
          "Ядаж нэг сэлбэг дээр тоо өөрчлөх, тайлбар бичих эсвэл ерөнхий тайлбар оруулна уу",
      };
    }
  }

  if (params.status === "approved" || params.status === "changes_requested") {
    const nextStepOrder = order_instances.current_step_order + 1;
    const { data: nextStep } = await supabase
      .from("order_steps")
      .select("id, step_order, required_approval_count")
      .eq("order_process_id", order_steps.order_process_id)
      .eq("step_order", nextStepOrder)
      .maybeSingle();

    if (nextStep) {
      const check = await validateNextStepReviewers(
        nextStep.id,
        nextStep.required_approval_count || 1,
      );

      if (!check.success) {
        return {
          success: false,
          error: `Дараагийн алхам руу шилжих боломжгүй: ${check.error}`,
        };
      }
    }
  }
...
async function createNextStepReviewers(
  ...
  const reviewers = Array.from(reviewerMap.entries()).map(
    ([profile_id, roleIds]) => ({
      order_instance_id,
  ...
  if (error) {
    console.error("CRITICAL: Failed to insert next step reviewers:", error);
    throw new Error("Дараагийн шатны шалгагчдыг бүртгэхэд алдаа гарлаа.");
  }
}
...
async function validateNextStepReviewers(
  next_step_id: number,
  ...
  if (!roles || roles.length === 0) {
    return {
      success: false,
      error: "Дараагийн алхамд Role тодорхойлогдоогүй байна.",
    };
  }
  ...
  for (const { role_id } of roles) {
    ...
    profiles?.forEach((p) => p.profile_id && uniqueProfiles.add(p.profile_id));
  }

  if (uniqueProfiles.size < required_count) {
    return {
      success: false,
      error: `Дараагийн алхамд ${required_count} хүн батлах ёстой ч системд ${uniqueProfiles.size} хүн байна.`,
    };
  }
