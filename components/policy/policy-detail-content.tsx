  ...
  return (
    <Accordion
      ...
      {policy.sections.map((section) => {
        const sectionMarkers = revisionMarkers.filter(
          (marker) =>
            marker.section_id === section.id &&
            marker.target_type === "section",
        );
        const shouldStrike = sectionMarkers.some((marker) =>
            ...
            <Card className="gap-0 overflow-hidden p-0">
              <AccordionTrigger className="border-b border-border bg-muted/30 px-4 py-3 hover:no-underline">
                <div className="flex min-w-0 flex-1 flex-col gap-1 text-left">
                  <div className="flex min-w-0 items-baseline gap-3">
                    ...
                    <h3
                      className={cn(
                        ...
                        shouldStrike && "line-through decoration-2",
                      )}>
                      {section.text}
                    </h3>
                  ...
                  <SectionRevisionLinks markers={sectionMarkers} />
                </div>
              </AccordionTrigger>
    ...
    <div className="flex flex-col gap-1">
      {markers.map((marker) => (
        <span
          ...
          className="rounded border bg-background px-2 py-1 text-[11px] font-normal text-muted-foreground">
          {marker.legal_act.act_number} тушаалаар{" "}
          {getRevisionChangeActionLabel(marker.change_action).toLowerCase()}
          {marker.change_note ? `: ${marker.change_note}` : ""}
        </span>
      ))}
    </div>
  );
