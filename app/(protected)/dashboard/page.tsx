  ...
  const orderStatCards = orderStats
    ? [
        ...
        },
        {
          ...
          value: orderStats.inProgress + orderStats.pending,
          icon: Clock,
      ...
      {policyOverview ? (
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <StatCard
              title="Нийт журам"
              value={policyOverview.totalPolicies}
              icon={FileText}
              color="blue"
              trend="+2 энэ сар"
            />
            <StatCard
              title="Нийт заалт"
              value={policyOverview.totalClauses}
              icon={BookOpen}
              color="violet"
            />
            <StatCard
              title="Нийт үнэлгээ"
              value={policyOverview.totalRatings}
              icon={BarChart3}
              color="cyan"
            />
            <StatCard
              title="Дундаж оноо"
              value={policyOverview.averageScore}
              icon={Star}
              color="amber"
              suffix="/ 5"
            />
...
function StatCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-sm transition-shadow hover:shadow-md">
      <div
        className={`absolute left-0 top-0 h-0.5 w-full ${c.bar} opacity-60`}
      />
